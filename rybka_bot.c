...
POLE **pobierzZPliku(int *m, int *n, char *path)
{
...
void zapiszdopliku(POLE **WskaznikNaPlansze, int m, int n)
{
    ...
    for(int i=0; i<IloscGraczy; i++)
        fprintf(plik,"%s %d %d\n", ListaGraczy[i], i+1, RybyGraczy[i]);
    fclose(plik);
}
...
void pobierzgraczyiryby(POLE **WskaznikNaPlansze, char *path_to_previous_phase)
{
 //   printf("Podaj nazwe pliku z poprzednim stanem gry.\nJesli zaczynasz nowe gre wpisz \"nowa\". ");
    char *NazwaPliku = path_to_previous_phase;
    char nowa[]="nowa";
    if(path_to_previous_phase == NULL)
        scanf("%s",NazwaPliku);
...
void tryb_automatyczny(POLE **WskaznikNaPlansze)
{
    for(int k=0;; k++)
    {
        int x,y;
...
int main(int argc, char **argv)
{
    ...
    /*
    WskaznikNaPlansze = pobierzZPliku(&m,&n, argv[1]);
 
    pobierzgraczyiryby(WskaznikNaPlansze, "nowa");
    drukujPlansze(WskaznikNaPlansze,m,n);
